...
export default function ProductForm({setNotification,setToast,refresh,setSelected,product,productTypes,assignedAttributes}){
    
    const [name,setName] = useState(product.name)
    const [date,setDate] = useState(product.created_at)
    const [type,setType] = useState(product.productType)
    const [attrs,setAttrs] = useState([...product.assignedAttributes])
    const [open,setOpen] = useState(false)
    const [addItem,setAddItem] = useState(assignedAttributes[0])

    function editItem(){
       
        axios.post(process.env.REACT_APP_SERVER+'/product/update',{
            id:product._id,
            name,
            created_at:date,
          ...
          .catch(err => console.log('err on updating products',err))
    }
           ...
           />
            ...
            >
                <>
                {
                    productTypes.map(
                        (type,index) => (
                            <option key={index+'_form_product_product_types_'+type._id} id={''+type._id}>{type.name}</option>
                        )
                    )
                }
                </>
                ...
                >
                    {
                        assignedAttributes.map(
                            (attr,index) => (
                                <option key={index+'_form_product_select_assigned_attr_'+type._id} id={'form_assigned_attr__'+attr._id}>{attr._id}</option>
                            )
                        )
                    }
                </select>
            ...
            <div className="flex flex-col w-8/10">
                    {
                        attrs?.map(
                            (attr,index) => (
                                <div key={index+'form_product_assigned_attr_'+attr._id} className="flex my-4 mx-auto ">
